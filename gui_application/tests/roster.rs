...
use gui_application::api::roster::{
    RetrieveAllAvailableStudentsOutcome, StudentPositionDto, StudentSummaryDto,
};
...
fn summaries_of(positions: Vec<StudentPosition>) -> RetrieveAllAvailableStudentsOutcome {
    FakeSam::default()
...
fn listed_positions(positions: Vec<StudentPosition>) -> Option<Vec<StudentPositionDto>> {
    match summaries_of(positions) {
        RetrieveAllAvailableStudentsOutcome::Success { students } => Some(
            students
                .into_iter()
                .map(|summary| summary.position)
                .collect(),
        ),
        RetrieveAllAvailableStudentsOutcome::Failure { .. } => None,
    }
}
...
fn only_musicians_have_an_instrument_name() {
    ...
    let RetrieveAllAvailableStudentsOutcome::Success { students } = result else {
